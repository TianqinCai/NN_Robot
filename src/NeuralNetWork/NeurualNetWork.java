...
public class NeurualNetWork implements NeuralNetInterface {
    ...
    private static double[] mOutputNeuronBiasWeights = new double[MAX_OUTPUT_NEURONS];
    private static double[] mPreviousOutputNeuronBiasWeights = new double[MAX_OUTPUT_NEURONS];
 ...
 * @param argA Integer lower bound of sigmoid used by the output neuron only.
    ...
    public void initializeWeights() {
        ...
        for(i = 0; i < mNumHidden; i++)
        {
            for(j = 0; j < mNumOutputs; j++)
            {
                ...
                mOutputNeuronBiasWeights[j] = random.nextDouble() - 0.5;
            }
        }
    ...
    private void updateWeight(){
        int h, o, i;
        double[] newOutputNeuronBiasWeights = new double[MAX_OUTPUT_NEURONS];
        ...
        for(o = 0; o < mNumOutputs; o++){
            newOutputNeuronBiasWeights[o] =
                    mOutputNeuronBiasWeights[o] +
                            calculateWeightDelta(1.0,
                                    mOutputNeuronErrors[o],
                                    mOutputNeuronBiasWeights[o],
                                    mPreviousOutputNeuronBiasWeights[o]);
        ...
        mHiddenOutputWeights = newHiddenOutputWeights.clone();
        mInputHiddenWeights = newInputHiddenWeights.clone();
    ...
    public double[] outputFor(double[] X){
        ...
        //TODO: initialize unactivated to zero first
        for(o = 0; o < mNumOutputs; o++){
            mOutputUnactivatedValue[o] = 0.0;
            for(h = 0; h < mNumHidden; h++){
                mOutputUnactivatedValue[o] += mHiddenNeuronValues[h] * mHiddenOutputWeights[h][o];
            }
            mOutputUnactivatedValue[o] += (1.0 * mOutputNeuronBiasWeights[o]);
    ...
    public List<List<List<Double>>> getWeights(){
        ...
        for(o = 0; o < mNumOutputs; o++){
            subset = new ArrayList<>();
            subset.add(mOutputNeuronBiasWeights[o]);
            for(h = 0; h < mNumHidden; h++){
    ...
    public void setWeights(List<List<List<Double>>> weights){
        ...
        for( o = 0; o < mNumOutputs; o++){
            mOutputNeuronBiasWeights[o] = weights.get(WEIGTHS_HIDDEN_OUTPUT_INDEX).get(o).get(0);
            for(h = 0; h < mNumHidden; h++){
